...
with col4:
    fig1 = px.line(roas_data, x="month", y="ROAS", color="attributed_channel", title="ROAS Trend by Channel",
                   color_discrete_sequence=['#F9D773', '#F9A825', '#FFD966'])
...
with col7:
    fig4 = px.bar(first_order_channel, x="attributed_channel", y="new_customers", title="New Customers by Channel",
                  text_auto=".2s", color_discrete_sequence=['#F9A825'])
    st.plotly_chart(fig4, use_container_width=True)
with col8:
    fig5 = px.bar(discount_impact, x="discount_status", y="avg_order_value", title="Average Order Value by Discount",
                  text_auto=".2f", color_discrete_sequence=['#6B3F1D'])
    st.plotly_chart(fig5, use_container_width=True)
with col9:
    fig6 = px.bar(revenue_summary, x="customer_type", y="total_revenue", title="Revenue: New vs. Returning",
                  text_auto=".2s", color_discrete_sequence=['#F9D773', '#F9A825'])
    st.plotly_chart(fig6, use_container_width=True)

# === ROW 3: Monthly Trends ===
fig = go.Figure()
fig.add_trace(go.Bar(x=summary['month'], y=summary['total_price'], name='Revenue',
                     marker_color='#F9A825', yaxis='y'))
fig.add_trace(go.Scatter(x=summary['month'], y=summary['orders'], mode='lines+markers',
                         name='Orders', marker=dict(color='#6B3F1D'), yaxis='y2'))
fig.add_trace(go.Scatter(x=summary['month'], y=summary['new_customers'], mode='lines+markers',
                         name='New Customers', marker=dict(color='#E67E22'), yaxis='y2'))
fig.update_layout(
